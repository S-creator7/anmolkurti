...
const ProductPreviewModal = ({ product, isOpen, onClose }) => {
  const { currency, addToCart, products, setProducts } = useContext(ShopContext);
              ...
              <div className="flex gap-3 overflow-x-auto">
                {product.image.map((img, index) => (
                  <button
                    ...
                    className={`flex-shrink-0 w-16 h-16 rounded-lg overflow-hidden border-2 transition-all ${index === currentImageIndex ? 'border-hotpink-500' : 'border-gray-200'
                      }`}
                  >
                    <img
                      src={img}
            ...
            <div className="space-y-6">
                    ...
                    {product.sizes?.map((size) => {
                      ...
                      return (
                        <button
                          ...
                          className={`px-4 py-2 border-2 rounded-lg text-sm font-medium transition-all ${selectedSize === size && !sizeOutOfStock
              ...
              <div className="flex gap-4">
                <button
                  ...
                  className={`flex-1 py-3 px-6 rounded-xl font-medium transition-all duration-300 ${outOfStock || (product.hasSize && !selectedSize) || isSelectedSizeOutOfStock()
                ...
                <button
                  ...
                  className={`p-3 rounded-xl border-2 transition-all duration-300 ${isInWishlist(product._id)
