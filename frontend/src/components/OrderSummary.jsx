...
const OrderSummary = ({ checkoutMode }) => {
  const location = useLocation();
  const isDirectBuy = location.state?.directBuy;

  const { currency, cartItems, products, directBuyItem } = useContext(ShopContext);
  ...
  if (directBuyItem) {
    ...
    if (directBuyItem) {
      orderItems = [{
        ...
        size: directBuyItem.selectedSize,
        quantity: directBuyItem.quantity,
        hasSize: directBuyItem.hasSize,
        stock: directBuyItem.stock
      }];
    }
  } else {
  ...
  const hasStockIssues = orderItems.some(item =>
    isItemOutOfStock(item) || quantityExceedsStock(item)
  );
    ...
    <div className='bg-white rounded-lg border border-gray-200 p-6'>
      ...
      {orderItems.length === 0 ? (
        ...
        <div className='space-y-4'>
          ...
          <div className='border-t pt-4 mt-4'>
            <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-medium ${isDirectBuy ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'
              }`}>
              {isDirectBuy ? (
                <>
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
      ...
      <div className='min-w-80'>
        <CartTotal
          ...
          hasCriticalStockIssues={hasStockIssues}
        />
      </div>
    </div>
  );
